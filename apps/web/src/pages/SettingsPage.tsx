...
export function SettingsPage() {
  ...
  const previewStyle = useMemo(
    () => ({
      ...
      color: form.dark_mode ? '#f9fafb' : '#111827',
      border: `1px solid ${form.theme_color}`
    }),
    [form.bg_color, form.dark_mode, form.theme_color]
  );

  if (loading) {
    return <section>Yükleniyor...</section>;
  }
    ...
    <section>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, alignItems: 'start' }}>
        <div style={{ display: 'grid', gap: 10 }}>
          <label>
            İşletme Adı
            <input value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} maxLength={120} />
          </label>
          ...
          <label>
            Tema Rengi
            <input type="color" value={form.theme_color} onChange={(e) => setForm((prev) => ({ ...prev, theme_color: e.target.value }))} />
          </label>
        ...
        <aside style={{ borderRadius: 10, padding: 10, ...previewStyle }}>
          ...
          {form.logo_url ? (
            <img
              src={form.logo_url}
              alt="Logo"
              style={{ width: 72, height: 72, objectFit: 'cover', borderRadius: 8, border: `2px solid ${form.theme_color}` }}
            />
          ) : (
            <div style={{ width: 72, height: 72, borderRadius: 8, border: `2px dashed ${form.theme_color}` }} />
          )}
